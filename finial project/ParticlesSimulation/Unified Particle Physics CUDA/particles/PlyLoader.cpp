...
int CPLYLoader::LoadModel(char* filename)
{
	...
	if (pch != NULL)
	{
		...
		try
		{
			...
			/*mp_confidence = (float*)malloc(ftell(file));
			mp_intensity = (float*)malloc(ftell(file));*/
		...
		/*if (mp_confidence == NULL) return -2;
		if (mp_intensity == NULL) return -3;*/
		...
		if (file)
		{
			...
			// read vertices
			//int index_vertice = 0;
			m_renderPos = new vec3<float>[m_totalFaces*3];
			mp_face = new unsigned int[3 * m_totalFaces];
			for (int iterator = 0; iterator < this->m_totalConnectedPoints; iterator++)//3488
			{
				...
				sscanf(buffer, "%f %f %f %f %f %f %f %f %f %f", &x, &y, &z,
					&mp_vertexNorm[i], &mp_vertexNorm[i + 1], &mp_vertexNorm[i + 2],
					&mp_vertexRGB[i], &mp_vertexRGB[i + 1], &mp_vertexRGB[i + 2] , &alpha);
				mp_vertexXYZ[iterator * 3] = x * scale_factor;
				mp_vertexXYZ[iterator * 3 + 1] = y * scale_factor;
				mp_vertexXYZ[iterator * 3 + 2] = z * scale_factor;

				mp_vertexRGB[i] /= 255.0f;
				mp_vertexRGB[i + 1] /= 255.0f;
				mp_vertexRGB[i + 2] /= 255.0f;
				i += 3;
			}
			...
			for (int iterator = 0; iterator < this->m_totalFaces; iterator++)//6920
			{
				...
				if (buffer[0] == '3')
				{
					...
					mp_face[iterator * 3] = vertex1;
					mp_face[iterator * 3 + 1] = vertex2;
					mp_face[iterator * 3 + 2] = vertex3;
					...
					m_renderPos[i] = vec3<float>(mp_vertexXYZ[3 * vertex1], mp_vertexXYZ[3 * vertex1 + 1], mp_vertexXYZ[3 * vertex1 + 2]);
					m_renderPos[i + 1] = vec3<float>(mp_vertexXYZ[3 * vertex2], mp_vertexXYZ[3 * vertex2 + 1], mp_vertexXYZ[3 * vertex2 + 2]);
					m_renderPos[i + 2] = vec3<float>(mp_vertexXYZ[3 * vertex3], mp_vertexXYZ[3 * vertex3 + 1], mp_vertexXYZ[3 * vertex3 + 2]);
